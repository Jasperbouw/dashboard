...
const HEALTH_META: Record<ContractorHealth, { label: string; color: string; bg: string; order: number }> = {
  ...
  'on-track':         { label: 'Op koers',          color: 'var(--color-success)',   bg: 'var(--color-success-subtle)',   order: 3 },
  active:             { label: 'Actief',            color: 'var(--color-ink-muted)', bg: 'var(--color-surface-raised)',   order: 4 },
            ...
            {sorted.map(c => {
              ...
              return (
                ...
                >
                  ...
                  {/* Health */}
                  <td style={td}>
                    <Pill label={health.label} color={health.color} bg={health.bg} />
                  </td>
