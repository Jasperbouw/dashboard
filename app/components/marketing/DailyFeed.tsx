...
export function DailyFeed() {
  ...
  async function publishCreative(id: string) {
    setActing(id)
  ...
  async function rejectCreative(id: string, reason: RejectionReason, notes: string) {
    ...
    await fetch(`/api/marketing/creatives/${id}`, {
      method:  'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({ status: 'rejected', rejection_reason: reason, rejection_notes: notes || undefined }),
    })
    setActing(null)
    setActioned(prev => new Set([...prev, id]))
    setModalId(null)
  ...
  return (
    <>
      <RejectionModal
        ...
        onSubmit={(reason, notes) => { if (modalId) rejectCreative(modalId, reason, notes) }}
      />
